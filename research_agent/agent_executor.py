...
class ResearchAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs the ResearchAgent for 'The Poet and the Sage'."""

    def __init__(self, runner: Runner, agent_name: str = "research_agent"):
        self.runner = runner
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id=self.agent_name, new_message=new_message
        )
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(session_id, new_message):
            if event.is_final_response():
                ...
                # Optional: Validate research output format
                for p in parts:
                    if isinstance(p.root, TextPart) and not is_valid_research_json(p.root.text):
                        logger.warning("Invalid research output format detected")
            ...
            if not event.get_function_calls():
                ...
                if event.content and event.content.parts:
                    logger.debug(
                        "Received parts: %s",
                        [p.text[:80] for p in event.content.parts if p.text],
                    )
...
def is_valid_research_json(text: str) -> bool:
    try:
        obj = json.loads(text)
        return all(k in obj for k in ("summary", "timeline", "key_figures", "pivotal_themes"))
    except Exception:
        return False
